...
export default function Home() {
  // Забираем пользователя из Zustand-стора (0 запросов к базе!)
  const user = useUserStore((state) => state.user);

  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center text-white pb-16">
      {/* --- HERO SECTION --- */}
      <main className="w-full max-w-6xl px-4 mt-12 md:mt-20 flex flex-col md:flex-row items-center justify-between gap-12">
        ...
        {/* Правая колонка: Мокап с графиком */}
      ...
      <section className="w-full max-w-6xl px-4 mt-20">
        <div className="border-t border-slate-800 pt-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 w-full">
            <Link
              href="/student"
              className="flex items-center justify-center p-4 bg-slate-800 text-white font-bold rounded-2xl shadow-sm border border-slate-700 hover:border-yellow-400 transition-all text-center"
            >
              👨‍🎓 Кабинет Ученика
            </Link>
            <Link
